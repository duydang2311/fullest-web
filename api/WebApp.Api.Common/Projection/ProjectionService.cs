...
{
    private readonly ConcurrentDictionary<Type, TypeProjection> projectionCache = [];

    public IDictionary<string, object?> Project<T>(T instance, string fields)
        where T : notnull
    {
        var queue =
            new Queue<(
    ...
    public Expression<Func<T, T>> Project<T>(string fields)
    {
    ...
    private static Dictionary<string, object?> BuildFieldTreeSpan(string fields)
    {
